...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/patients/:code/streak", async (req, res) => {
    try {
      ...
      // Start from today or the most recent completion date
      for (let i = 0; i < uniqueDates.length; i++) {
        ...
        if (i === 0) {
          ...
          const daysDiff = Math.floor((previousDate!.getTime() - new Date(currentDate).getTime()) / (1000 * 60 * 60 * 24));
          if (daysDiff === 1) {
            streakCount++;
            previousDate = new Date(currentDate);
          } else {
      ...
      // Calculate max streak by checking all possible consecutive sequences
      let tempStreak = 0;
      let tempPreviousDate: Date | null = null;
      
      for (const dateStr of uniqueDates.reverse()) { // Process chronologically
        const currentDate = new Date(dateStr);
        
        if (tempPreviousDate === null) {
          tempStreak = 1;
        } else {
          const daysDiff = Math.floor((currentDate.getTime() - tempPreviousDate.getTime()) / (1000 * 60 * 60 * 24));
          if (daysDiff === 1) {
            tempStreak++;
          } else {
